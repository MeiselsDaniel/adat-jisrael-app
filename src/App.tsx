...
const [adminOpen, setAdminOpen] = useState(false)
  ...
  async function handleDeleteAccount() {
    const firstConfirmation = window.confirm(
      'Vill du ta bort ditt konto? Kontot och dina personuppgifter i appen raderas permanent. Detta går inte att ångra.',
    )
    ...
    const secondConfirmation = window.confirm(
      'Är du helt säker på att du vill ta bort kontot permanent?',
    )
    ...
    const password = window.prompt(
      'Ange ditt lösenord för att bekräfta kontoborttagningen.',
    )
    ...
    try {
      ...
      window.alert(
        'Kontot kunde inte tas bort. Kontrollera lösenordet och försök igen.',
      )
    }
  }
  ...
  useEffect(() => {
    ...
    function handleTouchStart(event: TouchEvent) {
      ...
      // Bara svep som börjar vid vänsterkanten.
      if (touch.clientX > 30) {
        startX = null
        startY = null
        return
    ...
    function handleTouchEnd(event: TouchEvent) {
      ...
      // Tydligt svep åt höger, inte vanlig scroll.
      if (
        deltaX < 70 ||
        deltaX < deltaY * 1.5
      ) {
